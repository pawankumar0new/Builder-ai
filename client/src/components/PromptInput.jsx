...
import { ArrowRightIcon, CloudUploadIcon, Loader2Icon, Mic, MicIcon } from 'lucide-react';
    ...
    if(varient=== "glass"){
        return(
            <form onSubmit={handleSubmit} className='max-w-2xl w-full bg-white/10 backdrop-blur-xl rounded-xl ring-1 ring-white/25 focus-within:ring-2 focus-within:ring-white/30 overflow-hidden mt-6 transition'>
                <textarea ref={textareaRef} value={value} onChange={(e)=>setValue(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder} disabled={loading} rows={3} className='w-full bg-transparent border-none outline-none focus:outline-none resize-none px-3 pt-3 pb-2 text-white placeholder:text-white/50' />
